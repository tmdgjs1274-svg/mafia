...
io.on('connection', (socket) => {
  ...
  socket.on('resolve_night', () => {
    clearFakeTimers();
    currentPhase = 'DAY';
    hasExecutedToday = false; // 새로운 낮 초기화
  ...
  socket.on('kill_player', (playerId) => {
    ...
    if (p && p.isAlive) {
      p.isAlive = false;
      hasExecutedToday = true; // 처형 완료
      
      const sideName = (p.role === 'mafia') ? '🔴 마피아' : '⚪ 시민';
      const msg = `투표 결과로 ${p.nickname}님이 처형되었습니다. (${p.nickname}의 진영: ${sideName})`;

      io.to(p.id).emit('player_died');
      broadcastPlayerUpdate();

      io.emit('phase_change', { phase: 'DAY', resultMsg: msg });
    }
  });
